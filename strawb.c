...
void STRAWB_render(OBJ_ATTR* buffer, int camX, int camY, int* count, Actor *physics){
	...
	if (physics->ID & CHESTED_MASK)
	{
		...
		if (!(physics->flags & (COLLECTED_MASK | FOLLOWING_MASK))) {
			if (STRAWB_waitTime > 0){
				...
				signed char offset = (GAME_life * 4);
				
				if (physics->ID & WINGED_MASK)
					yPos += FIXED_MULT(INT_ABS(offset), 0x1000) - 0x400;
				else
					yPos += FIXED_MULT(INT_ABS(offset), 0x800) - 0x200;
			...
			if (physics->ID & WINGED_MASK)
			{
				...
				ATTR2_PALBANK(1) | (STRAWB_SP_OFFSET + 4) | ATTR2_PRIO(CHAR_PRIORITY + 1));
		...
		ATTR2_PALBANK(1) | STRAWB_SP_OFFSET | ATTR2_PRIO((physics->flags & (COLLECTED_MASK | FOLLOWING_MASK)) ? CHAR_PRIORITY : CHAR_PRIORITY + 1));
	...
	if (physics->ID & CHEST_MASK && !(physics->flags & (FLY_AWAY_MASK | FOLLOWING_MASK | COLLECTED_MASK)))
	{
		...
		ATTR2_PALBANK(2) | CHEST_SP_OFFSET | ATTR2_PRIO(CHAR_PRIORITY + 1));
...
void STRAWB_update(Actor *physics){
	...
	// If grabbed, but not collected, follow player
		...
		else if ((PHYS_actors[0].flags & ONGROUND_MASK && !(collide_rect(PHYS_actors[0].x, PHYS_actors[0].y, NORMAL_HITBOX_W, NORMAL_HITBOX_H) & COLLISION_STRAWB)) || (physics->ID & GOLDEN_MASK)) // Collect berry
		{
